...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Inisialisasi 12 bulan untuk tahun ini agar kurva selalu menampilkan semua bulan
      months.forEach(m => {
        chartDataMap[`${m} ${currentYear}`] = { periode: `${m} ${currentYear}`, Aset: 0, SHU: 0, VolumeUsaha: 0 }
      })

      if (mData && mData.length > 0) {
        mData.forEach((curr: any) => {
          const date = new Date(curr.created_at)
          const mIndex = date.getMonth()
          const year = date.getFullYear()
          const key = `${months[mIndex]} ${year}`
          
          if (!chartDataMap[key]) {
            chartDataMap[key] = { periode: key, Aset: 0, SHU: 0, VolumeUsaha: 0 }
          }
          
          chartDataMap[key].Aset += (curr.asset || 0) / 1000000
          chartDataMap[key].SHU += (curr.shu || 0) / 1000000
          chartDataMap[key].VolumeUsaha += (curr.volusaha || 0) / 1000000
        })
      }

      // Mengurutkan data berdasarkan waktu
      const sortedChartData = Object.values(chartDataMap).sort((a: any, b: any) => {
      ...
      setAdminChartData(sortedChartData)
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div
            ...
            className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex items-center gap-4 hover:border-indigo-400 hover:shadow-md cursor-pointer transition-all"
          >
          ...
          <div
            onClick={() => router.push('/admin/keragaan')}
            className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex items-center gap-4 hover:border-amber-400 hover:shadow-md cursor-pointer transition-all"
          >
          ...
          <div
            onClick={() => router.push('/admin/kesehatan')}
            className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex items-center gap-4 hover:border-green-400 hover:shadow-md cursor-pointer transition-all"
          >
